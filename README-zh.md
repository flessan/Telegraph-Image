...
11.嵌套**相册**：在公共工作区和后台中把对象整理成类似文件夹的层级。相册只是组织层——对象被归类、重命名或移动时，其 id、公开的 `/file/...` 链接、存储位置与审核状态都不会改变
...
相册在公共工作区（`/`）和远程存储控制台（`/admin.html`）中把对象整理为嵌套层级，默认可用，**不需要新的绑定或环境变量**。
...
- **本地优先**：在公共工作区可以完全离线创建相册并把暂存文件放进去，在按下 **Push** 之前不会上传任何内容；尚未保存到远端的相册会标记为“仅本地”。
